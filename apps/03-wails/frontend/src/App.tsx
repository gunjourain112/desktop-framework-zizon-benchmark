...
interface SystemData {
  ...
  process: {
    node: string;
    electron: string; // Used for "Wails" version
    chrome: string;
    platform: string;
    cpuModel: string;
  };
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (window.runtime) {
      window.runtime.EventsOn("system-update", (data: SystemData) => {
        setSystemData(data);
    ...
    // Cleanup not fully supported in simple global EventsOn without saving the handler reference,
  ...
  if (!systemData) {
    return (
      <Center h="100vh">
        <Stack align="center">
            <Text>Waiting for Wails Backend...</Text>
            {!window.runtime && <Text c="dimmed" size="xs">(Runtime not detected)</Text>}
        </Stack>
      </Center>
    );
  }
        ...
        <Grid.Col span={12}>
            <Paper withBorder p="xs" radius="md" bg="dark.8">
                <Group justify="space-between" px="md">
                    ...
                    <Group gap="lg">
                        <Text size="xs" c="dimmed">Go: <Text span c="white">{systemData.process.node}</Text></Text>
                        <Text size="xs" c="dimmed">Framework: <Text span c="white">{systemData.process.electron}</Text></Text>
                        <Text size="xs" c="dimmed">Engine: <Text span c="white">{systemData.process.chrome}</Text></Text>
