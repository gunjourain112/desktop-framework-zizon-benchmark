...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (systemData) {
      ...
      setCpuHistory((prev) => {
        const newHistory = [...prev, { time: now, load: systemData.cpu.currentLoad }];
  ...
  const memoryData = useMemo(() => {
    ...
    return [
      ...
      { name: 'Free', value: systemData.memory.free }, // available을 쓸 수도 있음
    ];
  }, [systemData]);
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={cpuHistory}>
                  ...
                  <Line
                    ...
                    strokeWidth={2}
                    dot={false}
                    isAnimationActive={false} // 실시간 업데이트 시 애니메이션이 어색할 수 있음
                  />
                </LineChart>
              </ResponsiveContainer>
            </div>
        ...
        <Grid.Col span={12}>
            <Paper withBorder p="xs" radius="md" bg="dark.8">
                <Group justify="space-between" px="md">
                    ...
                    <Group gap="lg">
                        <Text size="xs" c="dimmed">Node: <Text span c="white">{systemData.process.node}</Text></Text>
                        <Text size="xs" c="dimmed">Electron: <Text span c="white">{systemData.process.electron}</Text></Text>
                        <Text size="xs" c="dimmed">Chrome: <Text span c="white">{systemData.process.chrome}</Text></Text>
